...
class _HomeScreenState extends State<HomeScreen> {
  ...
  final List<Widget> _screens = [
    const HomeContent(),
    const Scaffold(body: Center(child: Text("Tickets Screen (Coming Soon)"))),
    const ProfileScreen(),
  ];
...
class HomeContent extends StatelessWidget {
        ...
        child: Column(
          ...
          children: [
            ...
            Padding(
              ...
              child: Row(
                children: [
                  Expanded(child: _buildCategoryCard(context, "Cari Travel", Icons.directions_bus, Colors.blue)),
                  const SizedBox(width: 16),
                  Expanded(child: _buildCategoryCard(context, "Cari Wisata", Icons.landscape, Colors.orange)),
                ],
              ),
            ),
            const Padding(
              padding: EdgeInsets.symmetric(horizontal: 16.0),
              child: Text("Promo Special", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            ),
            ...
            SizedBox(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) => _buildPromoCard(context, trips[index]),
              ),
            ),
          ],
        ),
  ...
  Widget _buildPromoCard(BuildContext context, Trip trip) {
    return GestureDetector(
      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => DetailScreen(trip: trip))),
      child: Container(
        width: 160,
        ...
        child: Column(
          ...
          children: [
            ClipRRect(
              ...
              child: Image.network(trip.image, height: 100, width: double.infinity, fit: BoxFit.cover),
            ),
            Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(trip.title, style: const TextStyle(fontWeight: FontWeight.bold), maxLines: 1),
                  Text(trip.price, style: const TextStyle(color: Colors.orange, fontWeight: FontWeight.bold)),
                ],
              ),
            )
          ],
